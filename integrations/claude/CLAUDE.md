...
- If output shows **user: unknown** (no recent human): ask **"Hi! I don't think we've met yet — what should I call you?"**; when they reply, run `brain interact --id user-<slug> --name <Name> --entity human --outcome success --notes "First meeting"` and `brain working --user user-<slug> --agent claude-code "Active user: user-<slug> (<Name>)"`.
