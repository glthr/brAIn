...
brain encode --user <id> --name <name> --agent claude-code --prompt "user's message" --response "your response"
```
...
```bash
brain working --user <id> --agent claude-code <text>                               # scratchpad (auto-expires)
brain fact --tags t1,t2 --user <id> --agent claude-code [--project /path] <text>  # store a fact manually
brain episode --tags t1,t2 --user <id> --agent claude-code [--project /path] <text> # record an episode manually
...
- If output shows **user: unknown** (no recent human): **STOP.** Ask **"Hi! I don't think we've met yet — what should I call you?"** and wait; when they reply, run `brain interact --id user-<slug> --name <Name> --entity human --outcome success --notes "First meeting"` and `brain working --user user-<slug> --agent claude-code "Active user: user-<slug> (<Name>)"`.
- Known user: greet **"Hi {name}! (If this isn't you, just let me know who you are.)"** and use the context output in your reply.
- If they say they are someone else: run `brain contacts --type human` to find or add them, then `brain working --user <id> --agent claude-code "Active user: <id> (<name>)"`.
...
**You MUST run `brain encode` after every single response to the user.** This includes short acknowledgments ("OK", "Done"), follow-up answers, error messages, and long technical replies. No interaction may be left unencoded.

Send the conversation turn to the brain by executing:

```bash
brain encode --user <user-id> --name <Name> --agent claude-code --prompt "the user's message" --response "your full response"
```

Run this command every time. Do not skip it.

The brain will automatically:
- Extract and store an episode summary
- Extract and store any reusable facts
- Log the interaction with outcome (and valence for agents only; humans are not scored)
...
- **Cross-project patterns** -- when storing a fact or episode that applies across all projects (e.g. a general coding principle, a user preference), use `--project common` to mark it as project-agnostic and universally reusable.
- **Use the context output** -- The single `brain context` run gives you facts, episodes, and traits; use them before acting and in your response. Reference past work, user preferences, and learned patterns.
- The brain file persists across all sessions and projects.
- Consolidation and profile inference happen automatically in the background.
- Trust the brain's output -- if it says you've seen something before, reference that experience.
- **Topic extraction** -- When extracting topics, think broadly: include synonyms, related concepts, and domain-specific terms. For example, "API" might also search for "endpoint", "REST", "HTTP".
