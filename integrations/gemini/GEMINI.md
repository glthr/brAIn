...
brain skill <goal or intent>                   # search procedures by goal; use when user asks for a task that may match an existing/emerging skill
...
brain encode --user <id> --name <name> --agent gemini --prompt "user's message" --response "your response"
```
...
```bash
brain working --user <id> --agent gemini <text>                               # scratchpad (auto-expires)
brain fact --tags t1,t2 --user <id> --agent gemini [--project /path] <text>  # store a fact manually
brain episode --tags t1,t2 --user <id> --agent gemini [--project /path] <text> # record an episode manually
...
- If output shows **user: unknown** (no recent human): ask **"Hi! I don't think we've met yet — what should I call you?"**; when they reply, run `brain interact --id user-<slug> --name <Name> --entity human --outcome success --notes "First meeting"` and `brain working --user user-<slug> --agent gemini "Active user: user-<slug> (<Name>)"`.
- Known user: greet **"Hi {name}! (If this isn't you, just let me know who you are.)"** and use the context output in your reply.
- If they say they are someone else: run `brain contacts --type human` to find or add them, then `brain working --user <id> --agent gemini "Active user: <id> (<name>)"`.
...
**Running `brain encode` is the LAST mandatory step of every single turn.** It must happen before you consider your turn finished. Every turn ends with a `brain encode` shell command.

Execute:

```bash
brain encode --user <user-id> --name <Name> --agent gemini --prompt "the user's message" --response "your full response"
```
